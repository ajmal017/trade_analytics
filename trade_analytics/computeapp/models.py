...
class ComputeStatus(models.Model):
    ...
    Compute_status_choices = [
        ('Fail', 'Fail'), ('ToDo', 'ToDo'), ('Success', 'Success'),
        ('Running', 'Running')]
    ComputeStatus = models.CharField(
        choices=Compute_status_choices,
        max_length=30,
        null=True)

    # any message to give more info
    ...
    def make_newcompute(cls, ComputeSession_id):
        obj = cls(
            ComputeStarttime=getnowtime(),
            ComputeSession_id=ComputeSession_id,
            ComputeStatus='ToDo')
        obj.save()
        return obj
