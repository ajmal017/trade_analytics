...
def processQ_SyncPrice2Meta(
        inQ, BadStartQ, BadLastQ, NoDataQ, InActiveQ, DuplicatesQ):
    while True:
        try:
            id = inQ.get_nowait()
        except Empty:
        ...
        print stk.Symbol, stk.Lastdate, stk.Startdate, stk.LastPriceUpdate,\
            stk.Status, stk.Derived

    stkprices = dtamd.Stockprice.objects.filter(
        Symbol_id=stk.id).order_by('Date')
    if len(stkprices) > 0:
    ...
    if md.ComputeStatus_Stockdownload.objects.filter(
            Symbol=stk, Status='Fail').count() >= 10:
...
def SyncPrice2Meta():
    ...
    for i in range(mp.cpu_count() - 1):
        P.append(
            mp.Process(
                target=processQ_SyncPrice2Meta,
                args=(
                    inQ,
                    BadStartQ,
                    BadLastQ,
